...
def check_humble_cli() -> bool:
    ...
    """
    try:
        subprocess.run(["humble-cli", "--version"], capture_output=True, check=True)
        return True
    ...
    try:
        ...
        for line in lines:
            if "," in line:
                ...
                if len(parts) == 2:
                    bundles.append({"key": parts[0].strip(), "name": parts[1].strip()})
...
def download_item_format(
    bundle_key: str, item_number: int, format_name: str, output_dir: str
) -> bool:
