...
class ItemFormatRow(ListItem):
    ...
    def cycle_format(self) -> None:
        ...
        if not self.formats:
            return
        current_idx = self.formats.index(self.selected_format) if self.selected_format in self.formats else 0
...
class BundleListScreen(Container):
    ...
    def compose(self) -> ComposeResult:
        yield Static("📚 Humble Bundle Library", classes="header-text", id="screen-header")
    ...
    async def load_bundles(self) -> None:
        ...
        try:
            ...
            status.update(f"Found {len(self.bundles)} bundles. Use ↑↓ to navigate, Enter to select.")
        ...
        try:
            ...
            if not self.bundle_data['items']:
                ...
                if self.bundle_data.get('keys'):
                    ...
                    status = self.query_one("#details-status", Static)
                    status.update("[yellow]No items found in this bundle. Press ESC to go back.[/yellow]")
                    # Set focus to the ListView so ESC key works (ListView can be focused even when empty)
                    list_view.focus()
                return
            
            # Add header row
            header_text = f"{'#':>3} | {'Item Name':50s} | {'Formats':30s} | {'Size':>10s}"
            list_view.append(ListItem(Label(f"[bold]{header_text}[/bold]")))
            
            # Add items
            for item in self.bundle_data['items']:
                list_view.append(ItemFormatRow(
